...
接下来的4-9实现的是单容器下的HDFS，用多个容器实现hadoop集群的可以跳过这段，直接从步骤10开始
...
接下来的10-18是用多个容器实现hadoop集群，总体思路是这样的，我们先用一个包含 Hadoop 的镜像进行配置，配置成集群中所有节点都可以共用的样子，然后再以它为原型生成若干个容器，构成一个集群
...
13.需要补的免密操作：
  ...
  再将root/.ssh下的文件复制一份给home/hadoop/,ssh下，并在root下把id_rsa操作：
    ...
    chmod 777 id_rsa变成其他用户可读，chmod 666 known_hosts让其他用户可写，这样就可以了
